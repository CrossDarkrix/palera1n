...
class palera1n(object):
    ...
    def sshrd_sh(self, arg, arg2='', arg3=''):
        ...
        if not arg == '' and not arg == 'boot':
            ...
            subprocess.run('{} pwn'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'gaster')), shell=True)
            subprocess.run('{} -e -s {} -m {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'img4tool'), os.path.join('ramdisk_shsh', '{}.shsh'.format(self.sshrd_check)), os.path.join(self.default_path, 'ramdisk_work', 'IM4M')), shell=True)
            ...
            subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), re.sub('<.*?[string]>', '', re.search('.+[{}].*iBSS[.].*'.format(self.sshrd_replace), BuildManifest).group()).replace('\t','').replace('\n', '').replace('/', os.sep), self.ipsw_url), shell=True)
            subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), re.sub('<.*?[string]>', '', re.search('.+[{}].*iBEC[.].*'.format(self.sshrd_replace), BuildManifest).group()).replace('\t','').replace('\n', '').replace('/', os.sep), self.ipsw_url), shell=True)
            subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), re.sub('<.*?[string]>', '', re.search('.+[{}].*DeviceTree[.].*'.format(self.sshrd_replace), BuildManifest).group()).replace('\t','').replace('\n', '').replace('/', os.sep), self.ipsw_url), shell=True)
            if self.os_type == 'Darwin':
                self.plutil_stdout = subprocess.Popen('/usr/bin/plutil -extract "BuildIdentities".0."Manifest"."RestoreRamDisk"."Info"."Path" xml1 -o - BuildManifest.plist', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0].decode()
                subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), os.path.join('Firmware', '{}.trustcache'.format(re.sub('(<.*?[string]>)', '', self.plutil_stdout.split('<string>')[-1].replace('\n', '')))), self.ipsw_url), shell=True)
            if self.os_type == 'Linux':
                self.plist_buddy_stdout = subprocess.Popen('{} BuildManifest.plist -c "Print BuildIdentities:0:Manifest:RestoreRamDisk:Info:Path"'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'PlistBuddy')), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0].decode()
                subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), os.path.join('Firmware', '{}.trustcache'.format(self.plist_buddy_stdout.replace('\n', '').replace('"', ''))), self.ipsw_url), shell=True)
            subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), re.sub('<.*?[string]>', '', re.search('.*kernelcache.release[.].*', BuildManifest).group()).replace('\t','').replace('\n', ''), self.ipsw_url), shell=True)
            if self.os_type == 'Darwin':
                subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), re.sub('(<.*?[string]>)', '', self.plutil_stdout.split('<string>')[-1].replace('\n', '')), self.ipsw_url), shell=True)
            if self.os_type == 'Linux':
                subprocess.run('{} -g {} {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'pzb'), self.plist_buddy_stdout.replace('\n', '').replace('"', '').replace('"', ''), self.ipsw_url), shell=True)
            ...
            if self.sshrd_check == '0x7001':
                option_check
            if option_check == '1':
                subprocess.run('%s %s %s -b "rd=md0 debug=0x2014e wdt=-1 -restore" -n' % (os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'iBoot64Patcher'), os.path.join(self.default_path, 'ramdisk_work', 'iBEC.dec'), os.path.join(self.default_path, 'ramdisk_work', 'iBEC.patched')), shell=True)
            else:
            ...
            if self.os_type == 'Darwin':
                subprocess.run('{} -i {} -o {} -M {} -T rtsc'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'img4'), '{}.trustcache'.format(re.sub('(<.*?[string]>)', '', self.plutil_stdout.split('<string>')[-1].replace('\n', ''))), os.path.join(self.default_path, 'sshramdisk', 'trustcache.img4'), os.path.join(self.default_path, 'ramdisk_work', 'IM4M')), shell=True)
                subprocess.run('{} -i {} -o {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'img4'), re.sub('(<.*?[string]>)', '', self.plutil_stdout.split('<string>')[-1].replace('\n', '')), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), shell=True)
            if self.os_type == 'Linux':
                subprocess.run('{} -i {} -o {} -M {} -T rtsc'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'img4'), os.path.join(self.default_path, 'ramdisk_work', '{}.trustcache'.format(self.plist_buddy_stdout.replace('\n', '').replace('"', ''))), os.path.join(self.default_path, 'sshramdisk', 'trustcache.img4'), os.path.join(self.default_path, 'ramdisk_work', 'IM4M')), shell=True)
                subprocess.run('{} -i {} -o {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'img4'), os.path.join(self.default_path, 'ramdisk_work', self.plist_buddy_stdout.replace('\n', '').replace('"', '')), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), shell=True)
            if self.os_type == 'Darwin':
                ...
                if not arg2 == 'rootless':
                    with zipfile.ZipFile(os.path.join('ramdisk_Pogo', 'Pogo.zip'), 'r') as pogo_ipa:
                        pogo_ipa.extractall(os.path.join(self.default_path, 'ramdisk_work', 'Pogo')+'/.')
                    with zipfile.ZipFile(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'Pogo.ipa'), 'r') as pogo:
                        pogo.extractall(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'Pogo')+'/.')
                    try:
                        shutil.rmtree('/tmp/SSHRD/usr/local/bin/loader.app')
                    except:
                        pass
                    try:
                        shutil.copytree(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'Pogo', 'Payload', 'Pogo.app'), '/tmp/SSHRD/usr/local/bin/loader.app')
                    except:
                        pass
                ...
                subprocess.run('hdiutil resize -sectors min {}'.format(os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), shell=True)
            if self.os_type == 'Linux':
                if os.path.exists(os.path.join('ramdisk_other', 'ramdisk.tar.gz')):
                    ...
                    subprocess.run('{} {} grow 300000000'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'hfsplus'), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    subprocess.run('{} {} untar {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'hfsplus'), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg'), os.path.join('ramdisk_other', 'ramdisk.tar')), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    if not arg2 == 'rootless':
                        with zipfile.ZipFile(os.path.join('ramdisk_Pogo', 'Pogo.zip'), 'r') as pogo_ipa:
                            pogo_ipa.extractall(os.path.join(self.default_path, 'ramdisk_work', 'Pogo')+'/.')
                        with zipfile.ZipFile(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'Pogo.ipa'), 'r') as pogo:
                            pogo.extractall(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'Pogo')+'/.')
                        os.makedirs(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'uwu', 'usr', 'local', 'bin'), exist_ok=True)
                        shutil.copytree(os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'Pogo', 'Payload', 'Pogo.app'), os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'uwu', 'usr', 'local', 'bin', 'loader.app'))
                        subprocess.run('{} {} rmall usr/local/bin/loader.app'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'hfsplus'), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        subprocess.run('{} {} addall {}'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'hfsplus'), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg'), os.path.join(self.default_path, 'ramdisk_work', 'Pogo', 'uwu')), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        subprocess.run('{} {} mv /usr/local/bin/loader.app/Pogo /usr/local/bin/loader.app/Tips'.format(os.path.join(self.default_path, 'ramdisk_{}'.format(self.os_type), 'hfsplus'), os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            try:
                pyimg4(['im4p', 'create', '-i', '{}'.format(os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.dmg')), '-o', '{}'.format(os.path.join(self.default_path, 'ramdisk_work', 'ramdisk.im4p')), '-f', 'rdsk'])
            except SystemExit:
                pass
            try:
    ...
    def main(self):
        ...
        if self.palera1n_OS_check == '1':
            ...
            if not os.path.exists(os.path.join(self.default_path, 'blobs', '{}-{}.shsh2'.format(self.device_id, self.ramdisk))):
                ...
                subprocess.run('{} -g "{}" "{}"'.format(os.path.join(self.binary_path, 'pzb'), re.sub('<.*?[string]>', '', re.search('.*kernelcache.release[.].*', BuildManifest).group()).replace('\t','').replace('\n', ''), self.ipsw_url), shell=True)
                shutil.move(re.sub('<.*?[string]>', '', re.search('.*kernelcache.release[.].*', BuildManifest).group()).replace('\t','').replace('\n', ''), os.path.join(self.default_path, 'palera1n_work', 'kernelcache'))
                dev_found = '0'
            ...
            if not os.path.exists(os.path.join(self.default_path, 'boot-{}'.format(self.device_id), 'ibot.img4')):
                ...
                subprocess.run('{} -g {} {}'.format(os.path.join(self.binary_path, 'pzb'), re.sub('<.*?[string]>', '', re.search('.+[{}].*iBSS[.].*'.format(self.model), BuildManifest2).group()).replace('\t','').replace('\n', ''), self.ipsw_url), shell=True)
                subprocess.run('{} decrypt {} iBSS.dec'.format(os.path.join(self.binary_path, 'gaster'), re.sub('<.*?[string]>', '', re.search('.+[{}].*iBSS[.].*'.format(self.model), BuildManifest2).group()).replace('\t','').split('/')[-1].replace('\n', '')), shell=True)
                print("[*] Downloading and decrypting iBoot")
                subprocess.run('{} -g {} {}'.format(os.path.join(self.binary_path, 'pzb'), re.sub('<.*?[string]>', '', re.search('.+[{}].*iBoot[.].*'.format(self.model), BuildManifest2).group()).replace('\t','').replace('\n', ''), self.ipsw_url), shell=True)
                subprocess.run('{} decrypt {} ibot.dec'.format(os.path.join(self.binary_path, 'gaster'), re.sub('<.*?[string]>', '', re.search('.+[{}].*iBoot[.].*'.format(self.model), BuildManifest2).group()).replace('\t','').split('/')[-1].replace('\n', '')), shell=True)
