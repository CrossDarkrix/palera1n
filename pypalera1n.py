...
def SSHRD(arg, arg2='', arg3=''):
    ...
    subprocess.run('{} work/iBEC.dec work/iBEC.patched -b "rd=md0 debug=0x2014e wdt=-1 "rd=md0 debug=0x2014e wdt=-1 `if [ -z "$2" ]; then :; else echo "$2=$3"; fi` `if [ "$check" = "0x8960" ] || [ "$check" = "0x7000" ] || [ "$check" = "0x7001" ]; then echo "-restore"; fi`" -n'.format(os.path.join(sshrd_work_dir, OS_Type, 'iBoot64Patcher')), shell=True)
